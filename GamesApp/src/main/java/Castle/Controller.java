...
public class Controller{
    ...
    public static String playerTurn(Player p){
        ...
        //if the player is up to his final three he is asked to choose one card
        if(p.getHand().size() < 4 && p.getHand().get(p.getHandSize() - 1).isFaceDown()){
            ...
            //if the player is down to his three face up cards they are flippd faced up and h continues with his turn.
            if(p.getHandSize() < 7 && p.getHandSize() > 3 && p.getHand().get(3).isFaceDown()){
            ...
            while(playerCards == null){
                System.out.println("Sorry, you typed in a card that you don't have, or you typed in the same card twice." +
                        " please try again.");
                playerCards = p.act();
            }
        }
    ...
    public static String computerTurn(Player cpu){
        ...
        //if the computer is down to is last three cards he chooses his last face down card
        if(cpu.getHandSize() < 4){
            if(cpu.getHand().get(0).isFaceDown()){
            ...
            System.out.println("The computer chose to play \n" + cpuCards.get(0));
            ...
            if(result.equals("true") || result.equals("goAgain"))
                ...
                System.out.println("The computer ate the pile.");
            }
            return result;
        }
        else{
            if(cpu.getHandSize() < 7 && cpu.getHandSize() > 3 && cpu.getHand().get(3).isFaceDown()){
            ...
            if(!canPlay && twoOrTen.size() != 0){
                System.out.println("The computer chose to play ");
                System.out.println(twoOrTen.get(0));
    ...
    //sees if the player or cpu has won. if he has it resets everything
    ...
    public static void main(String[] args){
        ...
        while(true){
            ...
            //computer set up
            //the computer looks for any 10's or 2's to play and keeps track of how many he has chosen
            for(int i = 0; i < 9; i++)
            ...
            //now the computer looks for his highest card to set as is top three
            cpu.sortt();
            while(tracker < 3){
            ...
            //it constantly checks to see if the player or computer has won
